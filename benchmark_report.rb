...
def report_format_single(detail)
  format('%<label>s completed in %<time>.4fs on average.',
         label: detail[:label],
         time: detail[:average_seconds])
end

def report_format_fastest(detail_fastest, detail_next_fastest)
  format('"%<fastest_label>s" was the fastest by %<speed_difference>.0f%% on average.',
...
def report_comparison(implementations, count)
  implementations = implementations.sort_by { |implementation| implementation[:total_seconds] }
  details = implementations.map { |implementation| build_details(implementation, count) }
...
def benchmark_report(count, iterations_per_count, test_data, implementations)
  ...
  puts
  report_comparison(implementations, count)
end
